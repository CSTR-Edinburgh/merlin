...
if test "$#" -ne 1; then
    echo "Usage: ./scripts/prepare_config_files_adapt.sh conf/global_settings_adapt.cfg"
    exit 1
fi
...
$SED -i s#'hidden_layer_size\s*:.*'#'hidden_layer_size: [1024, 1024, 1024, 1024, 1024, 1024]'# $duration_config_file
if [ "$AdaptationMethod" == "lhuc" ]
then
    $SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH_LHUC'\'','\''TANH_LHUC'\'','\''TANH_LHUC'\'', '\''TANH_LHUC'\'', '\''TANH_LHUC'\'', '\''TANH_LHUC'\'']'# $duration_config_file
elif [ "$AdaptationMethod" == "aux" ] || [ "$AdaptationMethod" == "fine_tune" ]
then
    $SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH'\'','\''TANH'\'','\''TANH'\'','\''TANH'\'','\''TANH'\'','\''TANH'\'']'# $duration_config_file
fi
#$SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH'\'', '\''LSTM'\'']'# $duration_config_file
if [ "$AdaptationMethod" == "lhuc" ] || [ "$AdaptationMethod" == "fine_tune" ]
then
    $SED -i s#'start_from_trained_model\s*:.*'#'start_from_trained_model:'${DurTrainedModel}# $duration_config_file
fi
if [ "$AdaptationMethod" == "lhuc" ]
then
    $SED -i s#'use_lhuc\s*:.*'#'use_lhuc:True'# $duration_config_file
fi
...
# [Architecture]
$SED -i s#'hidden_layer_size\s*:.*'#'hidden_layer_size: [1024, 1024, 1024, 1024, 1024, 1024]'# $acoustic_config_file
if [ "$AdaptationMethod" == "lhuc" ]
then
    $SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH_LHUC'\'', '\''TANH_LHUC'\'','\''TANH_LHUC'\'', '\''TANH_LHUC'\'', '\''TANH_LHUC'\'', '\''TANH_LHUC'\'']'# $acoustic_config_file
elif [ "$AdaptationMethod" == "aux" ] || [ "$AdaptationMethod" == "fine_tune" ]
then
    $SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH'\'','\''TANH'\'','\''TANH'\'','\''TANH'\'','\''TANH'\'','\''TANH'\'']'# $acoustic_config_file
fi
#$SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH'\'', '\''LSTM'\'']'# $acoustic_config_file
if [ "$AdaptationMethod" == "lhuc" ] || [ "$AdaptationMethod" == "fine_tune" ]
then
    $SED -i s#'start_from_trained_model\s*:.*'#'start_from_trained_model: '${AcTrainedModel}# $acoustic_config_file
fi
if [ "$AdaptationMethod" == "lhuc" ]
then
    $SED -i s#'use_lhuc\s*:.*'#'use_lhuc:True'# $acoustic_config_file
fi
