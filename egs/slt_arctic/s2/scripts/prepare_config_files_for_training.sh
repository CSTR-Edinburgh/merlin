...
duration_config_file=conf/dur_train_${Voice}.conf
...
if [ "$Vocoder" == "MAGPHASE" ]
then
    $SED -i s#'^label_align:.*'#'label_align_orig_const_rate: %(TOPLEVEL)s/experiments/'${Voice}'/acoustic_model/data/label_'${Labels}'\nlabel_align: %(TOPLEVEL)s/experiments/'${Voice}'/acoustic_model/data/label_'${Labels}'_var_rate'# $duration_config_file
else
    $SED -i s#'^label_align:.*'#'label_align: %(TOPLEVEL)s/experiments/'${Voice}'/acoustic_model/data/label_'${Labels}# $duration_config_file
fi
...
if [ "$Voice" == "slt_arctic_demo_magphase" ]
then
...
acoustic_config_file=conf/acous_train_${Voice}.conf
...
if [ "$Vocoder" == "MAGPHASE" ]
then
    $SED -i s#'^in_mgc_dir\s*:.*'#'in_acous_feats_dir: %(data)s/acoustic_feats'# $acoustic_config_file
    $SED -i s#'^in_lf0_dir\s*:.*'#' '# $acoustic_config_file
    $SED -i s#'^in_bap_dir\s*:.*'#' '# $acoustic_config_file
fi
...
if [ "$Vocoder" == "MAGPHASE" ]
then
    $SED -i s#'^label_align:.*'#'label_align_orig_const_rate: %(TOPLEVEL)s/experiments/'${Voice}'/acoustic_model/data/label_'${Labels}'\nlabel_align: %(TOPLEVEL)s/experiments/'${Voice}'/acoustic_model/data/label_'${Labels}'_var_rate'# $acoustic_config_file
else
...
if [ "$Vocoder" == "MAGPHASE" ]
then
    $SED -i s#'^mgc\s*:.*'#'mag: 60'# $acoustic_config_file
    $SED -i s#'^dmgc\s*:.*'#'dmag: 180'# $acoustic_config_file

    $SED -i s#'^bap\s*:.*'#'real: 45\ndreal: 135'# $acoustic_config_file
    $SED -i s#'^dbap\s*:.*'#'imag: 45\ndimag: 135'# $acoustic_config_file
...
if [ "$Vocoder" == "MAGPHASE" ] # Just removing unnecessary information from the base config file (not essential step).
then
    $SED -i s#'^framelength\s*:.*'#' '# $acoustic_config_file
    $SED -i s#'^fw_alpha\s*:.*'#'\# Not used by MagPhase (removed).'# $acoustic_config_file
    $SED -i s#'^minimum_phase_order\s*:.*'#' '# $acoustic_config_file
    $SED -i s#'^use_cep_ap\s*:.*'#' '# $acoustic_config_file
else
    if [ "$SamplingFreq" == "16000" ]
    then
...
if [ "$Vocoder" == "MAGPHASE" ]
then
    $SED -i s#'^output_features\s*:.*'#'output_features: ['\''mag'\'', '\''lf0'\'', '\''vuv'\'', '\''real'\'', '\''imag'\'']'# $acoustic_config_file
    $SED -i s#'^gen_wav_features\s*:.*'#'gen_wav_features: ['\''mag'\'', '\''lf0'\'', '\''real'\'', '\''imag'\'']'# $acoustic_config_file
fi
